...
module Impl<inStr/0 input> {
  ...
  string tileAt(int x, int y) { result = lines(y).charAt(x) }

  newtype TPos = MkPos(int x, int y) { exists(tileAt(x, y)) }
  ...
  class Pos extends TPos {
    ...
    string getTile() { result = tileAt(x, y) }

    string toString() { result = "(" + x + ", " + y + ")" }

    Dir getAnExit() { result = getAnExitForTile(getTile()) }

    int getX() { result = x }

    int getY() { result = y }

    predicate isStart() { getTile() = "S" }

    Pos getConnectedNeighbour(Dir d) {
      d = getAnExit() and
      (
        d = N() and result = MkPos(x, y - 1) and result.getAnExit() = S()
        or
        d = S() and result = MkPos(x, y + 1) and result.getAnExit() = N()
        or
        d = E() and result = MkPos(x + 1, y) and result.getAnExit() = W()
        or
        d = W() and result = MkPos(x - 1, y) and result.getAnExit() = E()
      )
    }

    Pos getAConnectedNeighbour() { result = getConnectedNeighbour(_) }

    Dir getALoopExit() {
      exists(Pos neighbour |
        neighbour.inLoop() and
        this.inLoop() and
        this.getConnectedNeighbour(result) = neighbour
      )
    }

    Pos getANonStartNeigbour() {
      result = getAConnectedNeighbour() and not result.isStart() and not this.isStart()
    }

    predicate isNextToStart() { getAConnectedNeighbour().isStart() }

    pragma[nomagic]
    predicate isReachableVia(Pos startNeighbour) {
      startNeighbour.isNextToStart() and
      startNeighbour.getANonStartNeigbour*() = this
    }

    predicate inLoop() {
      exists(Pos s1, Pos s2 | isReachableVia(s1) and isReachableVia(s2) and s1 != s2)
      or
      this.isStart()
    }

    int distFromStart() { result = distances(this) }

    /**
     * Is this position cnontained inside the loop.
     * This pretends the pipes run across the top and left edges so
     * must be combined with inLoop() to be correct for the puzzle.
     */
    boolean containedinLoop() {
      // This scans both directions, it would be more efficient to only scan one direction but the loss in effectiveness is small and we get to check that they agree
      x = 0 and
      y = 0 and
      if inLoop() then result = true else result = false
      or
      not this.inLoop() and result = MkPos(x - 1, y).(Pos).containedinLoop()
      or
      not this.inLoop() and result = MkPos(x - 1, y - 1).(Pos).containedinLoop()
      or
      (
        (
          if getALoopExit() = S()
          then result = MkPos(x - 1, y).(Pos).containedinLoop().booleanNot()
          else result = MkPos(x - 1, y).(Pos).containedinLoop()
        )
        or
        if getALoopExit() = E()
        then result = MkPos(x, y - 1).(Pos).containedinLoop().booleanNot()
        else result = MkPos(x, y - 1).(Pos).containedinLoop()
      )
    }
  }

  Dir getAnExitForTile(string tile) {
    tile = "|" and result = [N().(Dir), S()]
    or
    tile = "-" and result = [E().(Dir), W()]
    or
    tile = "L" and result = [N().(Dir), E()]
    or
    tile = "J" and result = [N().(Dir), W()]
    or
    tile = "F" and result = [S().(Dir), E()]
    or
    tile = "7" and result = [S().(Dir), W()]
    or
    tile = "S"
  }

  predicate start(Pos p) { p.isStart() }

  predicate next(Pos p, Pos q) { p.getAConnectedNeighbour() = q }

  int distances(Pos q) = shortestDistances(start/1, next/2)(_, q, result)

  int part1() { result = max(any(Pos p).distFromStart()) }

  int part2() { result = count(Pos p | p.containedinLoop() = true and not p.inLoop()) }
}

string testInput() { result = "..F7.\n.FJ|.\nSJ.L7\n|F--J\nLJ...\n" }
